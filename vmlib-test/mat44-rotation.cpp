...
// Referred to Computer Graphics exercise 3
...
TEST_CASE("4x4 rotation around X axis", "[rotation][mat44]")
{
	...
	SECTION("90 degrees")
	{
		auto const ninety_deg = make_rotation_x(3.1415926f / 2.f);

		REQUIRE_THAT(ninety_deg(0, 0), WithinAbs(1.f, kEps_));
		REQUIRE_THAT(ninety_deg(1, 1), WithinAbs(0.f, kEps_));
		REQUIRE_THAT(ninety_deg(2, 2), WithinAbs(0.f, kEps_));
		REQUIRE_THAT(ninety_deg(3, 3), WithinAbs(1.f, kEps_));
	}

	SECTION("-90 degrees")
	{
		auto const minus_ninety_deg = make_rotation_x(-3.1415926f / 2.f);

		REQUIRE_THAT(minus_ninety_deg(0, 0), WithinAbs(1.f, kEps_));
		REQUIRE_THAT(minus_ninety_deg(0, 1), WithinAbs(0.f, kEps_));
		REQUIRE_THAT(minus_ninety_deg(0, 2), WithinAbs(0.f, kEps_));
		REQUIRE_THAT(minus_ninety_deg(0, 3), WithinAbs(0.f, kEps_));
	}
}
...
TEST_CASE("4x4 rotation around Y axis", "[rotation][mat44]")
{
	...
	SECTION("90 degrees")
	{
		auto const ninety_deg = make_rotation_y(3.1415926f / 2.f);

		REQUIRE_THAT(ninety_deg(0, 0), WithinAbs(0.f, kEps_));
		REQUIRE_THAT(ninety_deg(2, 2), WithinAbs(0.f, kEps_));
		REQUIRE_THAT(ninety_deg(3, 3), WithinAbs(1.f, kEps_));
	}
	...
	SECTION("-90 degrees")
	{
		auto const minus_ninety_deg = make_rotation_y(-3.1415926f / 2.f);

		REQUIRE_THAT(minus_ninety_deg(0, 0), WithinAbs(0.f, kEps_));
		REQUIRE_THAT(minus_ninety_deg(0, 1), WithinAbs(0.f, kEps_));
		REQUIRE_THAT(minus_ninety_deg(0, 3), WithinAbs(0.f, kEps_));
	}
}
